  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = {
        ...
        // Mapping your existing form field
        // projectType -> product
      ...
      if (res.status >= 200 && res.status < 300) {
        ...
        const message =
          "Hi, I’m interested in Toilet Cubicles. Can you share details?";
    ...
    <div className="backdrop-blur-xs p-4 sm:p-5 rounded-2xl shadow-xl w-full max-w-xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
        ...
        <button
          type="submit"
          disabled={isSubmitting}
          className={`w-full py-3 rounded-lg font-semibold transition text-sm sm:text-base ${isSubmitting
        ...
        {/* Status */}
        {status && (
          <p className="text-center text-sm mt-2 font-medium">{status}</p>
        )}
      </form>
    </div>
  );
